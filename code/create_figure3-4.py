# -*- coding: utf-8 -*-
"""
Created on Thu Feb 23 09:43:19 2023
...
def open_cesm_land(file_path):
    ...
    SOM = np.squeeze(SOM)
    SOI=ds.SOILICE.sel(levgrnd=slice('0.007101')).groupby('time.month').mean('time').mean(('lat','lon')) # soil ice
    SOI = np.squeeze(SOI)
    SNC=ds.FSNO.groupby('time.month').mean('time').mean(('lat','lon')) # snow cover
    SNC=SNC*100
    SND=ds.H2OSNO.groupby('time.month').mean('time').mean(('lat','lon')) # snow depth
...
def open_cesm_land_sd(file_path):
    ...
    SOM = np.squeeze(SOM)
    SOI=ds.SOILICE.sel(levgrnd=slice('0.007101')).groupby('time.month').std('time').mean(('lat','lon')) # soil ice
    SOI = np.squeeze(SOI)
    SNC=ds.FSNO.groupby('time.month').std('time').mean(('lat','lon')) # snow cover
    SNC=SNC*100
    SND=ds.H2OSNO.groupby('time.month').std('time').mean(('lat','lon')) # snow depth
    SND=SND*0.1 #conversion from mm to cm
...
SOI_SD  = NOSL_SD[5] - SONL_SD[5]
SNC_SD  = NOSL_SD[6] - SONL_SD[6]
SND_SD  = NOSL_SD[7] - SONL_SD[7]
...
data_SNW_SD = {'Soil Liquid': SOM_SD.values,
            'Soil Ice': SOI_SD.values,
            'Snow Cover': SNC_SD.values}
...
for ds in df_data_SNW.columns:
    ...
    if ds == 'Snow Cover' :
        lines_snw += axp.plot(x, y, ls='-', color = 'blue', marker = 'o', lw=2)
           
    else:        
        lines_snw += ax3.plot(x, y, ls='-', color = cols_snw[k], marker = 'o', lw=2)
        k=k+1
    #axpp.plot(x, SND.values, ls='-', color = 'magenta', marker = '+', lw=1.0)
    #axpp.spines['right'].set_position(('outward',60))
...
for i, var in enumerate(df_data_SNW.columns):
    ...
    if var == 'Snow Cover':
        axp.errorbar(x, df_data_SNW[var], yerr=np.abs(df_data_SNW_SD[var]), fmt='-o', color='blue',
                     markersize=1, capsize=3, elinewidth=2, capthick=2)
    else:  
        ax3.errorbar(x, df_data_SNW[var], yerr=np.abs(df_data_SNW_SD[var]), fmt='-o', color=['lightseagreen','purple'][i],
                 markersize=1, capsize=3, elinewidth=2, capthick=2)
...
axp.set_ylabel('Fraction of ground covered by snow (%)', fontsize=26, color='blue')
...
gv.set_titles_and_labels(ax3, lefttitle="(c) Soil Moisture State & Snow Cover", lefttitlefontsize=30)
...
axp.tick_params(axis='y', colors='blue')

ax1.legend(lines, namelist, ncol=1, loc = 'upper left', labelcolor='linecolor', frameon = False)
ax2.legend(lines_lnd, namelist_lnd, ncol=1, loc = 'upper left', labelcolor='linecolor', frameon = False)
ax3.legend(lines_snw, namelist_snw, ncol=2, loc = 'lower left', labelcolor='linecolor', frameon = False)
ax4.legend(lines_veg, namelist_veg, ncol=1, loc = 'upper left', labelcolor='linecolor', frameon = False)

# Add a suptitle
fig.suptitle('Eurasia', fontsize=48, y=0.95)
#fig.suptitle('North America', fontsize=48, y=0.95)

fig.savefig('C:/Users/Jesse/Dropbox (UFL)/Jesse/Manuscript/figures/figure3.jpg', bbox_inches='tight', pad_inches = 0.1)
fig.savefig('C:/Users/Jesse/Dropbox (UFL)/Jesse/Manuscript/figures/figure3.pdf', bbox_inches='tight', pad_inches = 0.1)

#fig.savefig('C:/Users/Jesse/Dropbox (UFL)/Jesse/Manuscript/figures/figure4.jpg', bbox_inches='tight', pad_inches = 0.1)
#fig.savefig('C:/Users/Jesse/Dropbox (UFL)/Jesse/Manuscript/figures/figure4.pdf', bbox_inches='tight', pad_inches = 0.1)
